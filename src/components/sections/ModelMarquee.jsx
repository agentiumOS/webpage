...
export default function ModelMarquee() {
  return (
    <div className="w-full overflow-hidden bg-slate-900 border-y-4 border-emerald-500 py-6 relative z-20">
        ...
        >
          ...
          {[...usps, ...usps].map((usp, i) => (
            <div key={i} className="flex items-center gap-4 group">
              <div className="w-2 h-2 bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,0.8)] group-hover:scale-150 transition-transform"></div>
              <span className="font-mono text-lg font-bold text-white tracking-widest group-hover:text-emerald-400 transition-colors uppercase">{usp}</span>
            </div>
          ))}
        </motion.div>
      </div>
